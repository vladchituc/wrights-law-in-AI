...
3. **Closed-model FLOP estimates are imputed.** Epoch flags many closed frontier-class releases as `>1e25 FLOP` without publishing precise estimates. The `IMPUTED_FRONTIER` block in the script gives explicit values for 15 such models, drawing on Epoch blog posts, public lab statements, and consistency with neighboring releases. All imputed values are documented in the script with rationale. *Sensitivity*: doubling all 15 imputed values, halving them, or dropping them entirely (using only Epoch-published numbers) shifts the Wright exponent by at most 0.05 (range 0.77–0.82 across all scenarios) and Sahal's identity stays within 3 days of close in every case. R² is 0.91 throughout. The imputed numbers are cosmetic, not load-bearing.
